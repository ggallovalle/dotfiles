...
var type = {
  connected:
    /^(\S+) connected (?:(\d+)x(\d+)\+(\d+)\+(\d))*\s*(\(\w+\))*\s*(\w*)/,
  disconnected: /^(\S+) disconnected/,
  mode: /^\s+(\d+)x([0-9i]+)\s+(\(\w+\))\s+([0-9]+\.[0-9]+)MHz\s+((\s*[\+]*[\-]*\w*\s)*)/,
  dimension_horizontal:
    /^\s+h\:\s+width\s+(\d+)\s+start\s+(\d+)\s+end\s+(\d+)\s+total\s+(\d+)\s*skew\s*(\w)\s*clock\s*([0-9]*.[0-9]*)/,
  dimension_vertical:
    /^\s+v\:\s+height\s+(\d+)\s+start\s+(\d+)\s+end\s+(\d+)\s+total\s+(\d+)\s*clock\s*([0-9]*.[0-9]*)/,
};
...
function parse(data) {
  ...
  lines.forEach(function (line) {
    ...
    if ((tmp = type.connected.exec(line))) {
      ...
      result[last_connection].modes[last_mode - 1].dimensions.horizontal =
        dimension_h;
      ...
      result[last_connection].modes[last_mode - 1].dimensions.vertical =
        dimension_v;
    }
  });
  return result;
...
exec("xrandr --query", function (err, stdout) {
